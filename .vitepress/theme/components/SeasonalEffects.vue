...
.seasonal-container {
  ...
  &.interactive:hover .particle {
    // <-- 优化了交互逻辑的选择器
    transform: translate(calc((var(--mouse-x) - 50%) * -50px * var(--intensity) * (0.5 - abs(#{random-range(-0.5, 0.5)}))),
      calc((var(--mouse-y) - 50%) * -50px * var(--intensity) * (0.5 - abs(#{random-range(-0.5, 0.5)}))));
  }
    ...
    @for $i from 1 through 100 {
      &:nth-child(#{$i}) {
        ...
        @if $type =="petal" {
          ...
          transform: rotate(#{random-range(0, 360)}deg); // <-- 使用插值
    ...
    @for $i from 1 through 100 {
      &:nth-child(#{$i}) {
        ...
        @if $type =="glow" {
          ...
          background: radial-gradient(circle, rgba(255, 240, 150, random-range(0.4, 0.7)) 0%, rgba(255, 223, 100, 0) 70%);
          border-radius: 50%;
        ...
        @if $type =="sunray" {
          ...
          height: 2px;
          background: linear-gradient(to right, rgba(255, 223, 100, 0), rgba(255, 223, 100, random-range(0.2, 0.5)), rgba(255, 223, 100, 0));
          transform-origin: left center;
          transform: rotate(#{random-range(-45, 45)}deg); // <-- 使用插值
    ...
    @for $i from 1 through 100 {
      &:nth-child(#{$i}) {
        ...
        @if $type =="leaf" {
          ...
          background-color: if(math.random() > 0.7, if(math.random() > 0.5, #d97706, #b45309), if(math.random() > 0.5, #f59e0b, #ea580c));
