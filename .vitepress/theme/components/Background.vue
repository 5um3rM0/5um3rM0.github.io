<template>
  <Teleport to="body">
    ...
    <div v-if="backgroundType !== 'close'" :class="['background', backgroundType, themeValue]">
      <img v-if="backgroundType === 'image'" :src="dynamicBackgroundUrl" id="background-cover" class="cover" alt="background"
...
const dynamicBackgroundUrl = computed(() => {
  if (backgroundType.value === 'image' && backgroundUrl.value) {
    const separator = backgroundUrl.value.includes('?') ? '&' : '?';
    // 每次计算都添加一个时间戳，强制浏览器重新请求
    return `${backgroundUrl.value}${separator}t=${Date.now()}`;
  }
  return backgroundUrl.value; // 非图片模式或 backgroundUrl 为空时返回原始值
});
