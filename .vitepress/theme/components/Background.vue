  ...
  <Teleport to="body">
    ...
    <div :class="['background', backgroundType, themeValue, { 'is-loading': isLoading }]">
      ...
      <Transition name="fade-bg">
        <img v-if="(backgroundType === 'image' || backgroundType === 'selfie') && imageUrl" :key="imageUrl"
          :src="imageUrl" id="background-cover" class="cover" :style="{ objectFit: imageFit }" alt="background"
...
const imageUrl = ref('');
const isLoading = ref(false); // 用于控制加载状态的 class
let selfieInterval = null;

const onImageLoad = (e) => {
  e.target.classList.add("loaded");
  isLoading.value = false; // 图片加载完成，取消加载状态
};
...
const onImageError = () => {
  ...
  imageUrl.value = '';
  isLoading.value = false; // 加载失败，同样取消加载状态
};
...
const processImageUrl = async (url) => {
  ...
  isLoading.value = true; // 开始加载，进入“暗房”模式
  imageUrl.value = ''; // 先清空旧图片，确保 transition 触发
  ...
  if (!/\.(jpe?g|png|gif|webp)$/i.test(url)) {
    try {
      ...
      console.error("Failed to process background URL:", error);
      onImageError();
    }
  } else {
    imageUrl.value = url;
...
watch(backgroundType, (newType) => {
  stopSelfieMode();
  if (newType === 'image') {
    processImageUrl(backgroundUrl.value);
...
.background {
  ...
  // 在图片加载期间，强制使用柔和的深色背景
  &.is-loading {
    background-color: #18171d; // 深色模式的背景色
  }
